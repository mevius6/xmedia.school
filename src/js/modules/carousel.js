 ...
 * @class Carousel
 * ---
 * [repo]: https://github.com/argyleink/gui-challenges/tree/main/carousel
 *
 * @see
 * [GUI Challenges][repo]
 */
export default class Carousel {
  constructor(element) {
    this.elements = {
      ...
      scroller2:   element.querySelector(
        '.carousel__container:nth-child(2)'
      ) || null,
      snaps:      element.querySelectorAll('.carousel__slide'),
      // if null, elements will be generated in #createControl
    ...
    this.hasIntersected = new Set() // holds IO results used on scrollend
  ...
  goToElement({ scrollport, element }) {
    ...
    const n = this.#getElementIndex(element)+1
    ...
    if (this.elements.counter!=null) this.elements.counter.textContent = n
  }

  #updateControls() {
    const {
      lastElementChild: last, firstElementChild: first
  ...
  #createPagination() {
    ...
    nav.className = 'carousel__pagination'
  ...
  #createMarkerGallery({index, type, item}) {
    ...
    marker.className = 'carousel__control --gallery'
    marker.type = 'button'
    marker.role = 'tab'
  ...
  /* #createControls() {
    ...
    controls.className = 'carousel__control'

    // let prevBtn = this.#createControl('previous')
    let prevBtn = this.#createControl('prev')
  ...
  #createControl(btnType) {
    ...
    // control.className = `carousel__control --${btnType}`
    control.className = `carousel__control--${btnType}`
    control.setAttribute('aria-controls', 'carousel__control')
  ...
  #keypressAnimation(element) {
    element.style.animation = 'carousel--control-keypress 145ms var(--ease-in-out-quint)'
